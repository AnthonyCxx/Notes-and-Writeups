...
pointer that already has a value will not cause memory leak, but will deallocate the old memory (see an of how `operator=` is implemented [here](https://en.cppreference.com/w/cpp/memory/unique_ptr/operator=)).
