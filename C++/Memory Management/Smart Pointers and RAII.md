...
pointer that already has a value will not cause memory leak, but will deallocate the old memory (see an of how `operator=` is implemented [here](https://en.cppreference.com/w/cpp/memory/unique_ptr/operator=)). Similarly, assigning _nullptr_ to a unique pointer will also not leak memory, just deallocating the object.
