...
- To overload the ```++``` operator, you have to specify whether it is the pre- or post-increment operator. You do so by adding a dummy variable, which 
  will allow the compiler to differentiate. With no dummy variable, the operator is pre-increment; with a dummy variable, the operator is post-increment. 
  You can use an integer as this dummy value and just pass '0' every time. 
...
```
- Here, you can see a try, throw, and catch statement trio. Here's how it works: put any section of code that might cause an error (such as dividing by zero)
  into a try block. Then, write a conditional that throws a some data-type (usually a string) that evaluates to true if the program _would_ have run 
  into an error. This will immediately exit the try block whenever that error would have been thrown. Since you're throwing an error, you will also need to catch it.
  The 'catch' statement activates whenever an exception (error) is thrown, and which catch statement activates depends on the data-type being thrown. 
